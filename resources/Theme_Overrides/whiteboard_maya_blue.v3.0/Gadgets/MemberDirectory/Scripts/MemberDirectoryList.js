...
// start of MemberDirectoryListRenderer singleton
  ...
  if(MemberDirectoryListRenderer == null) 
  {
    ...
    MemberDirectoryListRenderer.stretchBlockId = 'idPrimaryContentBlock1Content';
  ...
  function setDirectoryOutput(html, encodeHtml)
  {
    ...
    var stretchBlock = $(MemberDirectoryListRenderer.stretchBlockId);
    if (stretchBlock && outputBlock)
    {
      if (stretchBlock.clientHeight < outputBlock.offsetHeight)
      {
        stretchBlock.style.height = stretchBlock.clientHeight + outputBlock.offsetHeight + 20 + 'px';
        //20 - magic number. I want to make height more.
      }
    }
  }
    ...
    var i, j;
	    ...
	    for (i = 1; i <= maxFilterQty; i++)
	    {
	    	optColumn=$("idF"+i+"Options");
		 	...
		 	if (optColumn)
		 	{
		 		options = BonaPage.$$('.optionLink', optColumn);
        ...
        if (options)
        {
          for (j = 0; j < options.length; j++)
          {
            ...
            name = filterOptionNames[i - 1]['opt' + options[j].getAttribute("fId", 0)].gtrim();
