(function()
{
  ...
  function WASearchResultsGadget(args)
  {
    ...
    function doSearch(pageNum)
    {
      ...
      }).done(function(data)
      {
		...
		else
		{
			...
			var url = WA.formatString(model.getUrlTemplate, encodeURIComponent(searchBox.val()), filter, pageNum);
    ...
    function refreshSelectedTypesCaption()
    {
      ...
      selectedTypesCaption.html(WA.formatString(model.text.contentTypeTemplate, text));
    }
    ...
    function renderPagerItems(jsonArr)
    {
      ...
      WA.foreach(jsonArr, function(item) 
      { 
        if (item.page)
        {
