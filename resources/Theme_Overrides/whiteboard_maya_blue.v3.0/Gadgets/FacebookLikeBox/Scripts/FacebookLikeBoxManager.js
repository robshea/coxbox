(function (window, WA, undefined)
{
  ...
  {
    window.WaFacebookLikeBoxManager = new FacebookLikeBoxManager();
  }

  function FacebookLikeBoxManager()
  {
    if (!WA) { return; }

    var pThis = this,
        typeName = 'WaFacebookLikeBoxManager';
    ...
    function createGadget(model, args)
    {
      ...
      gadgets[gadgetId] = new WaFacebookLikeBox(model, args);
      gadgets[gadgetId].Dispose.addHandler(onGadgetDispose);

      createFbRootElement();

      if (window.FB && FB.XFBML && FB.XFBML.parse) { return; }
